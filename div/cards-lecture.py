...
_hand_power_names_ = {1:'High card', 2:'One pair', 3:'Two pair', 4:'Three of a kind', 5:'Straight', 6:'Flush', 7:'Full house', 8:'Four of a kind', 9:'Straight flush'}
...
def calc_4_kind_power(value_groups):
    return [8,card_value(value_groups[0][0])]
...
def calc_3_kind_power(value_groups):
    return [4,card_value(value_groups[0][0])] # Assuming a standard 52-card deck, no other tie-breaker data is needed.
...
def power_test(hands, hand_size = 7):
    for i in range(hands):
        ...
        print "Hand: " , card_names(cards), '  Power: ', power, ' (', power_name(power[0]), ")\n"
